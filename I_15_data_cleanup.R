...
# Read in data and drop unnecesary columns
post_to_latlon <- read.csv("Postmile_to_latlon.csv")
flow_n <- read.csv("flow_2020_N_I_15.csv")
flow_s <- read.csv("flow_2020_S_I_15.csv")

flow_s <- subset(flow_s, select = -c(Freeway, Direction, Minimum, Maximum, X..Lane.Points))
flow_n <- subset(flow_n, select = -c(Freeway, Direction, Minimum, Maximum, X..Lane.Points))
...
flow_s <- flow_s %>% group_by(Postmile..Abs.) %>% 
  summarise(flow = sum(Mean), .groups = 'drop')
flow_n <- flow_n %>% group_by(Postmile..Abs.) %>% 
  summarise(flow = sum(Mean), .groups = 'drop')

# all = False because the ones that are not symmetric don't look like they follow the pattern of close exits.
...
calc_lat_lon <- function(pm, Abs_PM) {
  ...
  new_longitude <- above_weight * post_to_latlon$Longitude[post_to_latlon$Abs_PM == PM_station_above] + below_weight * post_to_latlon$Longitude[post_to_latlon$Abs_PM == PM_station_below]
  new_latitude <- above_weight * post_to_latlon$Latitude[post_to_latlon$Abs_PM == PM_station_above] + below_weight * post_to_latlon$Latitude[post_to_latlon$Abs_PM == PM_station_below]
